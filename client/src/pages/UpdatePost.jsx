    ...
    useEffect(() =>{
        const fetchPost = async () => {
            ...
            if(response.data.success === true){
                setValue("title", response.data.data.posts[0].title)
                setValue("category", response.data.data.posts[0].category)
                setValue("content", response.data.data.posts[0].content)
                setPrevImageUrl(response.data.data.posts[0].image)
                setPostIdToUpdate(response.data.data.posts[0]._id)
            }
        }
        fetchPost()
    },[postId])
            ...
            <form onSubmit={handleSubmit(Submit)} encType='multipart/form-data' className='flex flex-col gap-4'>
                ...
                {tempImageUrl ? (
                    <div className='rounded-xl overflow-hidden'>
                        ...
                        <div className='rounded-xl overflow-hidden'>
                            <img className='w-full ' src={prevImageUrl} alt="postImage" />
                        </div>
                    )
                }
